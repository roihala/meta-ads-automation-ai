...
| [`load_active_plans.py`](load_active_plans.py) | **(2026-05-13 PM, feedback loop)** Lightweight cross-run plan memory. Extracts the `תוכנית:` block from rationales of recent approved/executed approvals; returns the forward-looking steps (steps 2-3) per `(target_kind, target_id)`. The agent reads its prior conditional commitments and decides per-step whether the trigger is now met. Soft memory — no new table; full plan-status tracking deferred to migration 023+. **Bound by guardrail §39** `respect_active_plans` — see check_guardrails.py. |
