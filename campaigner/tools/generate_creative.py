...
def main() -> None:
    p = argparse.ArgumentParser(description="Generate an image creative and record it in the gallery.")
    p.add_argument("--business-id", required=True)
    p.add_argument("--prompt", required=True, help="base image prompt (English or Hebrew — Vertex handles both)")
    p.add_argument("--aspect-ratio", default="1:1", choices=VALID_ASPECTS)
    p.add_argument("--angle", choices=VALID_ANGLES, default=None, help="marketing angle tag for the gallery row")
    p.add_argument("--placement", choices=VALID_PLACEMENTS, default="feed")
    p.add_argument("--copy", default=None,
    ...
    p.add_argument("--output-dir", default="./generated_images",
    ...
    def _insert() -> dict:
        with get_connection() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                """,
                (
                    args.business_id, "image",
                    save_path, args.aspect_ratio, dimensions,
                    headline, primary_text, cta,
                    "imagen", args.prompt, args.angle, args.placement,
                ),
            )
            return cur.fetchone()
    ...
    try:
        ...
        emit_runtime_error(f"creative_gallery insert failed: {e}. Image saved at {save_path}", exc=e)
        return
